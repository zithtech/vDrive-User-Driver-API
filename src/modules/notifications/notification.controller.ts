...
export const NotificationController = {
  ...
  async sendNotification(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      let messageId;
      if (driverId) {
        messageId = await NotificationService.sendNotificationToDriver(driverId, title, body, data);
      } else if (userId) {
        messageId = await NotificationService.sendNotificationToUser(userId, title, body, data);
      }
   ...
   */
  async subscribeToCoupon(req: Request, res: Response, next: NextFunction) {
    try {
   ...
   */
  async unsubscribeFromCoupon(req: Request, res: Response, next: NextFunction) {
    try {
      const { userId, couponCode, fcmToken } = req.body;

...
export class TripVerificationService {
   ...
   */
  private static async signVerificationUrls(
    verification: TripVerification | null
    ...
    return {
      ...
      car_images: verification.car_images
        ? await Promise.all(verification.car_images.map(signSingle))
        : undefined,
    } as TripVerification;
  }
  /**
  ...
  static async submitPhotos(data: CreateTripVerificationInput): Promise<TripVerification> {
    logger.info(
      `Submitting trip verification photos for driver: ${data.driver_id}, trip: ${data.trip_id}`
    );
  ...
  static async getPendingVerifications(): Promise<TripVerification[]> {
    ...
    return await Promise.all(
      verifications.map((v) => this.signVerificationUrls(v) as Promise<TripVerification>)
    );
  }
    ...
    if (verification.status === 'approved') {
      ...
      try {
        ...
        if (driverFcmToken) {
          ...
          await DriverNotifications.tripVerificationApproved(
            driverFcmToken,
            verification.trip_id || ''
          );
        }
      ...
      emitToRoom(
        `driver_${verification.driver_id}`,
  ...
  static async getVerificationDetails(verificationId: string) {
    ...
    if (profileSelfieDoc && profileSelfieDoc.document_url) {
      try {
        const parsed =
          typeof profileSelfieDoc.document_url === 'string'
            ? JSON.parse(profileSelfieDoc.document_url)
            : profileSelfieDoc.document_url;
  ...
  static async testForceVerifyDriver(driverId: string): Promise<void> {
    ...
    if (latest && latest.status === 'pending') {
      await TripVerificationRepository.updateStatus(
        latest.id,
        'approved',
    ...
    return await Promise.all(
      history.map(async (h) => {
        ...
        return {
          ...
          car_images: h.car_images
            ? await Promise.all(h.car_images.map((url) => signSingle(url)))
            : undefined,
        };
      })
    );

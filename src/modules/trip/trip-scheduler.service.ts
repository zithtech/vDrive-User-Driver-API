      ...
      for (const trip of result.rows) {
        ...
        if (diffMinutes >= 28 && diffMinutes <= 32 && !trip.reminders_sent?.thirty_min) {
          if (trip.driver_status === 'ONLINE') {
            ...
            if (messageId) {
              logger.info(
                `Successfully sent 30-minute reminder for trip ${trip.trip_id} to driver ${trip.driver_id}`
              );
              // Notify app via socket
              const { emitToRoom } = require('../../sockets/socket');
              emitToRoom(`driver_${trip.driver_id}`, 'SCHEDULED_REMINDER', {
            ...
            logger.info(
              `Skipping 30-minute reminder for trip ${trip.trip_id} as driver ${trip.driver_id} is OFFLINE.`
            );
          }
        }
        ...
        if (diffMinutes >= 8 && diffMinutes <= 12 && !trip.reminders_sent?.ten_min) {
          if (trip.driver_status === 'OFFLINE') {
            // Unassign
            logger.warn(
              `Auto-unassigning trip ${trip.trip_id} because driver ${trip.driver_id} is OFFLINE 10 mins before start.`
            );
            ...
            if (messageId) {
              logger.info(
                `Successfully sent 10-minute reminder for trip ${trip.trip_id} to driver ${trip.driver_id}`
              );
              // Notify app via socket
              const { emitToRoom } = require('../../sockets/socket');
              emitToRoom(`driver_${trip.driver_id}`, 'SCHEDULED_REMINDER', {
  ...
  async broadcastUpcomingScheduledRides() {
    try {
      ...
      for (const trip of result.rows) {
          ...
          const messageId = await NotificationService.sendNotification(
            ...
            `A scheduled ride is starting soon at ${trip.pickup_address}.`,
            {
              ...
              scheduled_start_time:
                trip.scheduled_start_time instanceof Date
        ...
        if (broadcastSuccess) {
          logger.info(
            `📡 Broadcasted "Starting Soon" for trip ${trip.trip_id} to ${eligibleDrivers.rows.length} drivers.`
          );
          await query('UPDATE trips SET last_broadcast_at = NOW() WHERE trip_id = $1', [
            trip.trip_id,
          ]);
        }
      }

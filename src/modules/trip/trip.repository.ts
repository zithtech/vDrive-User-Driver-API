...
export const TripRepository = {
  ...
  async findActiveTripByUserId(userId: string): Promise<any> {
    const result = await query(
     ...
     WHERE t.user_id = $1 
       ...
       AND (
         (t.booking_type = 'LIVE' AND t.trip_status IN ('ASSIGNED', 'ACCEPTED', 'VERIFICATION_PENDING', 'ARRIVING', 'ARRIVED', 'LIVE', 'DESTINATION_REACHED', 'WAITING', 'DAY_HALT', 'RETURN_STARTED', 'RETURN_REACHED'))
         OR
  ...
  async findActiveByDriverId(driverId: string): Promise<Trip | null> {
    const result = await query(
      ...
      LEFT JOIN users u ON t.user_id = u.id
      WHERE t.driver_id = $1
      AND t.trip_status IN ('ASSIGNED', 'ACCEPTED', 'VERIFICATION_PENDING', 'ARRIVING', 'ARRIVED', 'LIVE', 'DESTINATION_REACHED', 'WAITING', 'DAY_HALT', 'RETURN_STARTED', 'RETURN_REACHED')
      ORDER BY t.created_at DESC
      LIMIT 1;`,
      [driverId]
    );

...
const initializeFirebase = (): void => {
  ...
  try {
    ...
    if (process.env.FIREBASE_SERVICE_ACCOUNT) {
      ...
      throw new Error(
        'Neither FIREBASE_SERVICE_ACCOUNT nor FIREBASE_SERVICE_ACCOUNT_PATH environment variable is set'
      );
    }
...
// ─── Build FCM Message ────────────────────────────────────────────────────────
const buildMessage = (fcmToken: string, payload: NotificationPayload): admin.messaging.Message => {
...
// ─── Subscribe to Topic ───────────────────────────────────────────────────────
export const subscribeToTopic = async (fcmToken: string, topic: string): Promise<boolean> => {
  try {
...
export const unsubscribeFromTopic = async (fcmToken: string, topic: string): Promise<boolean> => {
  try {
